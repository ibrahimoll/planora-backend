...
- `129 passed`
- Confirmed after Step 29 Alembic Migration Setup.
- Step 29 Alembic head is `7562179d6e8d` after removing accidental autogenerated migration `34a07f3e8ab1_message.py`.
...
- `python -m alembic history` showed `<base> -> 2bf54f983173 -> 7562179d6e8d`.
- `python -m alembic heads` showed `7562179d6e8d` as the single head.
- Accidental autogenerated migration `34a07f3e8ab1_message.py` was removed after it attempted unsafe broad schema changes.
- Full regression with `TEST_DATABASE_URL` set to `planora_test_db` passed: `129 passed`.
...
### Step 30 Admin Authentication Foundation — Completed

Completed:

- Created separate Next.js admin dashboard frontend repository.
- Connected admin dashboard frontend to the existing FastAPI backend.
- Fixed Next.js app route structure so root app routes are used.
- `/` redirects to `/login`.
...
- Forgot password redirects to reset password after sending reset code.
- Reset password supports email, 6-digit code, new password, confirm password, and frontend validation matching backend password rules.
- Login UI was simplified: removed the right-side generic “Secure command access” panel and changed the button text to `Login`.

### Step 30 Dashboard Visual Direction — Approved

Approved frontend style:
...
- Keep useful motion only: page transitions, subtle reveal sections, hover states, active sidebar state, and a clean loading state.

Important style distinction:

- Admin dashboard keeps the clean dark SaaS/admin-product style.
- Future public landing page follows the separate future landing-page rules below.

### Step 30 Dashboard Shell — Current Layout Rules

Current admin-dashboard shell behavior:
...
- `dashboard-scale` should be applied to the dashboard content region only, not around the topbar, because zoom can interfere with sticky behavior.
- Avoid custom inner scrollbars inside cards unless absolutely necessary. The user specifically does not want a custom scroll inside the project detail card.

### Step 30.8 — Admin Users Page — Added
...
- Users page uses real backend API data through `lib/api.ts`.
- Includes search, role filter, active/inactive filter, and email verification filter.
- Shows filtered user counts calculated from returned backend users.
- Shows active/admin/verified counts.
- Shows dense user list/table-style account management UI.
- Selecting a user loads detail and activity panel.
- Supports activate/deactivate and promote/demote actions through protected admin endpoints.
- Prevents self-action buttons from being enabled in the UI.
- Includes loading, empty, error, and success states.
- Uses the approved clean dark SaaS dashboard style.

Important note:

- Users page is account-management focused, so table/list density is acceptable there.

### Step 30.9 — Admin Projects Page — In Progress / Manual Integration
...
Important backend/frontend contract:

- Frontend must call `GET /admin/projects`.
- Status filter query parameter must be `status`, not `status_filter`.
- Project type query parameter must be `project_type`, not `type` or `projectType`.
- Status update body must be `{ "status": "in_progress" }` using one of `not_started`, `in_progress`, `completed`, `on_hold`, or `cancelled`.
- Backend search currently matches project `title` and `description` only. The Projects page placeholder should say `Search title or description...`, not owner/team search, unless backend search is expanded later.

Current frontend implementation direction:

- Projects page should look different from Users page.
- Users page = account management list/table.
- Projects page = project portfolio/delivery board.
- Projects page should focus on progress, deadlines, blocked work, risk, and delivery status.
- Projects should be grouped vertically by status to keep cards visible and selectable.
- Avoid horizontal board layouts that hide projects off-screen.
- Project cards should show title, owner, project type, progress bar, risk badge, deadline status, completed/total task count, and blocked task count.
- Selected project opens a right-side/bottom detail panel depending on viewport.
- Detail panel should show project summary, owner, team/personal info, members count, deadline, updated date, task progress, task counts, and status update control.
- Detail panel must not have a custom inner scrollbar. It should grow normally with the page content.
- Selected project card should be visually obvious with a stronger teal border/ring.
- Filters should use fixed-height pills and a thin vertical divider between status filters and project type filters on wide screens.
...
Current frontend types added/needed in `types/admin.ts`:

- `AdminProjectStatus`
- `AdminProjectType`
- `AdminProjectOwner`
- `AdminProjectTeam`
- `AdminProjectTaskStats`
- `AdminProjectRisk`
- `AdminProjectSummary`
- `AdminProjectDetail`
- `AdminProjectStatusUpdateResponse`

Remaining confirmation before marking Step 30.9 complete:

- Run `npm run lint` in `admin-dashboard`.
- Run `npm run build` in `admin-dashboard`.
- Start frontend and backend locally.
- Confirm `/dashboard/projects` loads real projects.
- Confirm search works for project title/description.
- Confirm filters work for status and project type.
- Confirm selecting a project updates the detail panel.
- Confirm PATCH status update works and refreshes list/detail.
- Confirm no custom scrollbar appears inside the project detail card.
- Confirm sidebar and topbar remain visible while dashboard content scrolls.
- Commit/push admin-dashboard changes after verification.

### Remaining Step 30 Work

Next sub-steps after Step 30.9:
